...
void* BinsearchIdx(const void* key, const void* base, size_t input_number, size_t size, int(*Compare)(const void*, const void*));
...
int main()
{
	...
	int* result = (int*)BinsearchIdx(&search_number, arr, input_number, sizeof(int), Compare);
	if (result == NULL)
	{
		printf("검색 실패 \n");
	}
...
// bsearch 함수를 이용해 제일 앞 인덱스 찾는 함수 
void* BinsearchIdx(const void* key, const void* base, size_t input_number, size_t size, int(*Compare)(const void*, const void*))
{
	if (input_number == 0) return NULL; // 안전장치

	int arr_first = 0;
	int arr_last = input_number - 1;
	int count = 0;
	...
	while (arr_first <= arr_last)
	{
		...
		const int* arr_start = (const int*)base;
		const int* pos = (const int*)p;

		// p가 가리키는 값과 key를 비교하여 같으면 반복문 실행
		if (temp == 0)
		{
			// 중복값의 첫 번째 위치로 이동
			while (pos > arr_start && *(pos - 1) == *(const int*)key)
			{
				pos--;
			}
			return (void*)pos;
		}
		else if (temp == 1)	//key가 더 크면 왼쪽 탐색
		{
			arr_first = arr_mid + 1;
		}
		else	 // key가 더 작으면 오른쪽 탐색
			arr_last = arr_mid - 1;
	}
	//찾는 값 없을시 NULL 반환 
	return NULL;
...
int Compare(const void* arr_number, const void* search_nummber)
{
	...
	if (number_1 < number_2) return -1;  // key가 더 크면 왼쪽 탐색
	else if (number_1 > number_2) return 1; // key가 더 작으면 오른쪽 탐색
